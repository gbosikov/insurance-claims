...
extraction_use_rules = True:
  OCR text → rule_extractor.extract_by_rules() → ExtractionResult
...
✅ Дополнительные фичи (реализованы после основного pipeline):
  ...
  ✅ Devtools (/devtools — HTML форма ручной подачи, только dev)
   ...
   Шаг 20: ML-классификатор типов документов (после 600 примеров)
   ...
   Шаг 34: ML-классификатор [ждёт 600 подтверждённых примеров]
...
- **Свежая БД**: ревизии 0001→0006 применяются последовательно
- **Существующая БД** (до введения Alembic): 0001 видит таблицу `claims` и пропускается; 0002 идемпотентна; 0003–0006 используют `IF NOT EXISTS`
