...
# v3.2.0: coherence check расширен на receipt line_items (2 разреза: форма 100 + чеки)
PROMPT_VERSION = "decision/v4.0.0"
    ...
    "input_schema": {
        ...
        "properties": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "coverage_clarity": {
                            ...
                            "description": (
                                "Насколько ОДНОЗНАЧНО договор покрывает или исключает этот диагноз. "
                                "Оценивай ТОЛЬКО юридическую ясность договора, НЕ качество документов. "
                                "1.0 = договор прямо называет этот диагноз или его точную категорию. "
                                "0.7 = диагноз явно входит в покрытую категорию по иерархии МКБ-10. "
                                "0.4 = нет явного упоминания, интерпретируешь по смыслу категорий. "
                                "0.1 = договор молчит об этом случае, крайняя неопределённость."
                            ),
                        },
                    },
                    "required": ["icd10_code", "is_covered", "approved_amount", "coverage_clarity"]
                }
            },
            "line_items": {
                "type": "array",
            ...
            "summary":                  {
                "type": "string",
                "description": (
                    "სრული განჩინება ქართულ ენაზე: გადაწყვეტილება + დასაბუთება + სანდოობის დონე + შენიშვნები. "
                    "ეს ტექსტი მოხვდება Comment ველში ძირითად სისტემაში და წაიკითხავს ოპერატორი. "
                    "ОБЯЗАТЕЛЬНО пиши только на грузинском языке (ქართულად)."
                )
            },
        },
        "required": [
            "diagnoses", "total_approved", "deductible_applied",
            "final_payout", "requires_manual_review", "summary"
        ]
    }
}
...
Проверь связь услуг с диагнозами в ДВУХ разрезах:

1. МЕДИЦИНСКИЙ РАЗРЕЗ — услуги из формы 100 (doc_source='form_100'):
   Соответствуют ли назначенные процедуры диагнозам?
   Пример несоответствия: «МРТ позвоночника» при J06.9 (ОРВИ).

2. ФИНАНСОВЫЙ РАЗРЕЗ — услуги из чеков (doc_source начинается на 'receipt_'):
   Содержит ли чек услуги, медицински обоснованные для данных диагнозов?
   ФЛАГИРОВАТЬ (явно немедицинские или не связанные с диагнозом):
   - Массаж / Солярий при гипертонии, ОРВИ или любом острозаразном диагнозе
   - Косметология / Лазерная эпиляция / Антицеллюлитные процедуры
   НЕ ФЛАГИРОВАТЬ (стандартно обоснованные):
   - Консультация врача — обоснована при любом диагнозе
   - Анализ крови / ОАК / Биохимия (глюкозотолерантный тест, калий, альдостерон, A1c, и др.)
   - ЭКГ / ЭхоКГ — обоснованы при кардиологических диагнозах
   - УЗИ / Рентген — обоснованы если орган соответствует диагнозу
   - Препараты (эсциталопрам, метформин, статины и др.) — обоснованы если профиль диагнозов совпадает
   - Витамины и добавки (витамин D, B12, магний и др.) — обоснованы при дефицитных диагнозах (E55, E53, и др.)
   СТРОГО ЗАПРЕЩЕНО ФЛАГИРОВАТЬ как несоответствие:
   - Позиции с amount_currency ≠ GEL (IU, mg, ml, tab, ჯერ, ერთ) — они корректно
     исключены из финансовых расчётов правилом AMOUNT_CURRENCY. Это НЕ медицинское
     несоответствие, а просто отсутствие цены. НИКОГДА не пиши в coherence_flags
     что-либо о "денежной стоимости", "amount_currency", "не учитываются в расчётах".
   - Лабораторные исследования при профильных диагнозах (эндокринология, кардиология, гинекология)

Несоответствие → опиши конкретно ТОЛЬКО если услуга явно противоречит диагнозам.
НЕ отказывай автоматически — окончательное решение примет оператор.

ФИНАНСЫ: рассчитывай сумму с учётом coverage_pct и остатка remaining.
ПРАВИЛО AMOUNT_CURRENCY: в line_items поле amount_currency показывает тип значения:
  - "GEL" / "₾" → реальная денежная сумма → включай в финансовые расчёты
  - "IU" / "ერთ" / "ერთეული" → единицы витаминов/препаратов (например 5600 IU ≠ 5600 GEL)
  - "mg" / "ml" / "tab" / "ჯერ" → дозировка/количество → НЕ денежная сумма
  Для позиций с amount_currency ≠ GEL используй amount=0 в финансовых расчётах.
  Проверяй медицинскую обоснованность такой позиции, но не считай её стоимость.
COVERAGE_CLARITY: для каждого диагноза оценивай ТОЛЬКО юридическую ясность договора:
  1.0 = договор прямо упоминает этот диагноз или его точную категорию
  0.7 = диагноз явно входит в покрытую категорию по иерархии МКБ-10
  0.4 = явного упоминания нет, интерпретируешь по смыслу близких категорий
  0.1 = договор молчит, крайняя юридическая неопределённость
НЕ снижай coverage_clarity из-за: качества документов, подозрений на мошенничество,
размера суммы — это не твоя зона ответственности. Оценивай только текст договора.
SUMMARY: ОБЯЗАТЕЛЬНО пиши ТОЛЬКО на грузинском языке (ქართულად).
  ეს ტექსტი წაიკითხავს ძირითადი სისტემის ოპერატორი.
  შეიცავდეს: გადაწყვეტილება + დასაბუთება + სანდოობა + შენიშვნები.
  მაგ.: «ამბულატორული მკურნალობა. J06.9 — დაფარულია. გადასახდელი: 96 GEL.»
...
def check_sublimits(
    ...
    _GEL = frozenset({"GEL", "₾", "ლარი", "LARI", "ЛАРИ"})

    strictest = min(risks_with_sublimit, key=lambda r: r.sublimit)
    for item in line_items:
        # Пропускаем дозировочные позиции (IU/mg/ჯერ/ერთ) — они не денежные суммы
        currency = (getattr(item, "amount_currency", None) or "GEL").upper().strip()
        if currency not in _GEL:
            continue
...
# Штрафы data_score: детерминированные, только по флагам extraction.
# Сумма штрафов по всем флагам не уходит ниже 0 — clamp в compute_data_score().
_DATA_SCORE_PENALTIES: dict[str, float] = {
    "name_mismatch":        0.30,  # ФИО в разных документах не совпадают
    "birth_date_mismatch":  0.30,  # дата рождения не совпадает между документами
    "diagnosis_mismatch":   0.20,  # коды МКБ-10 в форме 100 и чеке расходятся
    "date_mismatch":        0.15,  # даты события расходятся более чем на 3 дня
    "missing_date":         0.15,  # дата события отсутствует в документах
    "missing_icd10":        0.15,  # нет кода МКБ-10 ни в одном документе
    "institution_mismatch": 0.10,  # название клиники не совпадает между документами
...
def compute_data_score(extraction: "ExtractionResult") -> float:
    """
    Детерминированный сигнал качества входных данных.
    Основан исключительно на флагах extraction — без AI-суждений.
    1.0 = данные полные и согласованные; 0.0 = критические расхождения.
    """
    score = 1.0
    flags = set(extraction.flags or [])
    for flag, penalty in _DATA_SCORE_PENALTIES.items():
        if flag in flags:
            score -= penalty
    return max(0.0, round(score, 3))


def compute_amount_gate(total_claimed: float, annual_limit: float) -> float:
    """
    Сигнал риска по размеру заявки относительно годового лимита.
    Крупная заявка требует более строгого порога — gate снижается.
    Пороги и скоры задаются через settings, не хардкодируются.
    """
    if annual_limit <= 0 or total_claimed <= 0:
        return 1.0
    ratio = total_claimed / annual_limit
    if ratio >= settings.decision_amount_gate_high_pct:
        return settings.decision_amount_gate_high_score
    if ratio >= settings.decision_amount_gate_medium_pct:
        return settings.decision_amount_gate_medium_score
    return 1.0
...
def build_routing_signal(
    ...
    """
    Итоговый сигнал маршрутизации = min(data_score, coverage_signal, amount_gate).

    min() позволяет диагностировать ПРИЧИНУ manual_review: слабый сигнал виден в breakdown.
    - data_score мал  → проблема в качестве/согласованности документов
    - coverage_signal мал → договор трудно интерпретировать для этого диагноза
    - amount_gate мал → заявка слишком крупная для автоодобрения

    Returns: (routing_signal, signal_breakdown dict для audit_log и devtools)
    """
    routing_signal = round(min(data_score, coverage_signal, amount_gate), 3)
    if data_score <= coverage_signal and data_score <= amount_gate:
        limiting = "data_score"
    elif coverage_signal <= amount_gate:
        limiting = "coverage_signal"
    else:
        limiting = "amount_gate"
    breakdown = {
        "routing_signal":  routing_signal,
        "data_score":      round(data_score, 3),
        "coverage_signal": round(coverage_signal, 3),
        "amount_gate":     round(amount_gate, 3),
        "limiting_factor": limiting,
    }
    return routing_signal, breakdown
            ...
            "line_items":    [
                    {
                        ...
                        "amount":         li.amount,
                        "amount_currency": li.amount_currency or "GEL",
                    }
                    for li in extraction.event.line_items
                ],
...
Первичное решение: is_covered={target.is_covered}, approved_amount={target.approved_amount}, coverage_clarity={target.coverage_clarity}
...
с цитатой из договора в contract_reference. Остальные поля заполни нулями/пустыми."""
    ...
    target.coverage_clarity = refined.get("coverage_clarity", target.coverage_clarity)
    ...
    log.info(
        ...
        icd10_code=target.icd10_code,
        coverage_clarity_before=before.get("coverage_clarity"),
        coverage_clarity_after=target.coverage_clarity,
    )
    return True
    ...
    *,
    ...
    with AuditTimer() as timer:
        ...
        try:
            ...
            return ClaimDecision(
                ...
                summary=(
                    f"მოვლენის თარიღის ამოცნობა ვერ მოხერხდა: «{extraction.event.date}». "
        ...
        try:
            ...
            return ClaimDecision(
                ...
                summary=(
                    f"პოლისის წლიური ლიმიტი ამოწურულია: ნაშთი {e.remaining} {e.currency}. "
        ...
        if not check_claim_filed_in_time(submission_date, event_date):
            ...
            return ClaimDecision(
                ...
                overall_confidence=1.0,
                summary=(
                    f"განაცხადი შეტანილია მოვლენიდან {delta_days} დღის შემდეგ "
        ...
        if (
            ...
            return ClaimDecision(
                ...
                overall_confidence=1.0,
                summary=(
                    f"მოვლენის თარიღი {event_date} სცდება პოლისის მოქმედების პერიოდს "
            ...
            if risks_limits.policy_start_date is None:
                ...
                if not check_waiting_period(
                    ...
                    return ClaimDecision(
                        ...
                        overall_confidence=1.0,
                        summary=(
                            f"მოვლენა მოხდა პოლისის {days_since_start}-ე დღეს, "
                            f"მოლოდინის პერიოდი {settings.decision_default_waiting_period_days} დღეა (დაგეგმილი მომსახურება). "
                            "საჭიროა ოპერატორის შემოწმება."
                        ),
                        prompt_version=PROMPT_VERSION,
                        model_version=get_active_model_name(),
        ...
        # Несоответствие услуг диагнозам → штраф к coverage_signal + manual_review.
        ...
        if settings.decision_coherence_check_enabled:
            ...
            _GENERIC_PHRASES = (
                ...
                # Фильтруем ложные coherence-флаги от AMOUNT_CURRENCY правила:
                # Claude не должен флагировать отсутствие цены как медицинское несоответствие
                "денежной стоимости",
                "финансовых расчет",
                "amount_currency",
                "не учитываются",
        ...
        if coherence_flags:
            log.info("medical_coherence_flags", claim_id=str(claim_id), flags=coherence_flags)

        # ── Три сигнала маршрутизации (новая архитектура) ─────────
        # calibration_factor применяется ТОЛЬКО к coverage_signal (AI-часть).
        # data_score и amount_gate — детерминированные, калибровка не нужна.
        ...
        # coverage_signal = min coverage_clarity по всем диагнозам × calibration_factor
        # + штраф за медицинскую несогласованность (Шаг 21)
        raw_clarities = [
            float(d.get("coverage_clarity", 0.5))
            for d in (raw.get("diagnoses") or [])
        ]
        raw_coverage_signal = min(raw_clarities) if raw_clarities else 0.5
        coverage_signal = max(0.0, min(1.0, raw_coverage_signal * calibration_factor))
        if coherence_flags:
            coverage_signal = max(
                0.0, coverage_signal - settings.decision_coherence_confidence_penalty
            )

        data_score = compute_data_score(extraction)
        amount_gate = compute_amount_gate(
            total_claimed=extraction.event.total_claimed or 0.0,
            annual_limit=risks_limits.annual_limit or 0.0,
        )
        routing_signal, signal_breakdown = build_routing_signal(data_score, coverage_signal, amount_gate)

        log.info(
            "routing_signal_computed",
            claim_id=str(claim_id),
            **signal_breakdown,
        ...
        uncertain = [
            ...
            if d.coverage_clarity < settings.decision_second_pass_confidence_threshold
        ]
        if uncertain and not raw.get("requires_manual_review"):
            second_pass_applied = await _second_pass_diagnosis(
                client=llm_client,
                target=min(uncertain, key=lambda d: d.coverage_clarity),
                enriched=enriched,
        ...
        decision = ClaimDecision(
            ...
            overall_confidence=routing_signal,
    ...
    await write_audit_entry(
        ...
        confidence={
            ...
            # raw_coverage_signal обязателен: калибровка считается от некалиброванного
            # значения, иначе фактор компаундился бы при каждом обновлении (Шаг 27)
            "raw_coverage_signal": raw_coverage_signal,
            "calibration_factor": calibration_factor,
            "signal_breakdown":   signal_breakdown,
        },
