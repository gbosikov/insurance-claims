...
# v1.4.0: documents[] — классификация типа документа встроена в extraction tool-call
#         (заменяет regex-предположение до вызова LLM); doc_index нейтральная нумерация
#         в user-сообщении, doc_source остаётся отдельной семантической нумерацией чеков
PROMPT_VERSION = "extraction/v1.4.0"
    ...
    "input_schema": {
        ...
        "properties": {
            "documents": {
                "type": "array",
                "description": (
                    "Классификация КАЖДОГО документа из пакета по индексу (в порядке, "
                    "в котором они перечислены в user-сообщении: ДОКУМЕНТ #1, #2, ...). "
                    "Один объект на документ, в том же порядке, без пропусков."
                ),
                "items": {
                    "type": "object",
                    "properties": {
                        "doc_index": {
                            "type": "integer",
                            "description": "1-based индекс документа (ДОКУМЕНТ #{n})"
                        },
                        "doc_type": {
                            "type": "string",
                            "enum": [
                                "form_100", "id_document", "receipt",
                                "discharge_summary", "lab_result", "prescription", "other",
                            ],
                            "description": "Тип документа. 'other' если не удаётся уверенно определить тип."
                        },
                        "doc_type_confidence": {
                            "type": "number",
                            "minimum": 0.0,
                    ...
                    "required": ["doc_index", "doc_type", "doc_type_confidence"]
                }
            },
            "insured": {
                "type": "object",
        ...
        "required": ["documents", "insured", "event", "extraction_confidence"]
    }
}
...
КЛАССИФИКАЦИЯ ДОКУМЕНТОВ (documents[]):
══════════════════════════════════════════════════════
- Каждый документ в пакете помечен как "ДОКУМЕНТ #{n}" — заполни ОДИН объект
  в documents[] на каждый номер, без пропусков, doc_index = {n}.
- Классифицируй по ДОМИНИРУЮЩЕМУ типу содержимого ВСЕГО документа, а не по
  наличию отдельных полей внутри него:
    form_100           — направление/акт: диагноз, врач, учреждение, список услуг
    discharge_summary  — выписной эпикриз: ретроспективный нарратив ЗАВЕРШЁННОГО
                          случая лечения (в отличие от form_100 — направления)
    lab_result         — результаты лабораторных анализов (таблица показателей,
                          референсные значения)
    prescription       — ОТДЕЛЬНЫЙ аптечный рецептурный бланк
    receipt            — чек/квитанция/счёт об оплате
    id_document        — паспорт/удостоверение личности
    other              — нечитаемо, не медицинский документ, либо ты не уверен
                          (confidence < 0.5) — НЕ пытайся угадать через силу
- ВАЖНЫЙ НЮАНС: форма 100 очень часто содержит внутри себя не только диагноз,
  но и назначенные медикаменты/дозировки — это НЕ делает документ рецептом.
  Если документ выглядит как форма 100/направление (есть направление, диагноз,
  врач, учреждение) — классифицируй как form_100, ДАЖЕ ЕСЛИ в нём перечислены
  медикаменты. prescription — только для самостоятельных рецептурных бланков
  без признаков формы 100. Список медикаментов внутри form_100 извлекается как
  обычно через line_items — отдельный тип документа для этого не нужен.
- doc_index — нейтральная нумерация документов в пакете (ДОКУМЕНТ #1, #2, ...).
  Это ОТДЕЛЬНАЯ система от doc_source в line_items/receipt_summaries
  ('receipt_1', 'receipt_2', 'form_100') — НЕ путай их. doc_source по-прежнему
  строится по семантике (порядковый номер ЧЕКА, а не документа в пакете).
...
def _build_user_message(ocr_results: list[OCRResult]) -> str:
    ...
    Документы нумеруются нейтрально (ДОКУМЕНТ #1, #2...) БЕЗ предположения типа —
    тип определяет сам Claude в documents[] (см. SYSTEM_PROMPT). Это заменяет
    прежнюю схему, где regex-классификатор навязывал лейбл (ФОРМА 100/ЧЕК)
    ДО того, как LLM вообще видел текст.
    """
    parts = []
    for idx, result in enumerate(ocr_results, start=1):
        parts.append(
            f"=== ДОКУМЕНТ #{idx} (doc_index={idx}, OCR confidence: {result.avg_confidence:.2f}) ===\n"
            f"{result.full_text}"
        )
...
async def _persist_extracted_data(
    ...
    form_100, discharge_summary, lab_result, prescription
                → insured + event (общий клинический срез — единый LLM-вызов
                  на всю заявку не разделяет извлечение по типу документа)
    receipt     → line_items + total_claimed
    id_document → insured
    other       → {} (неуверенная классификация — нечего атрибутировать)
    + as_seen_in_document: значения из cross_document для этого типа документа
      (заполнено только для form_100/id_document/receipt — см. EXTRACTION_TOOL).
    """
    ...
    for ocr in ocr_results:
        ...
        if ocr.doc_type in (
            DocType.FORM_100, DocType.DISCHARGE_SUMMARY,
            DocType.LAB_RESULT, DocType.PRESCRIPTION,
        ):
            data: dict[str, Any] = {
        ...
        elif ocr.doc_type == DocType.ID_DOCUMENT:
        ...
        else:  # DocType.OTHER — неуверенная классификация, нечего атрибутировать
            data = {}
...
async def extract_claim_data(
    ...
    if settings.extraction_use_rules:
        # Regex-классификатор — единственный источник типа документа в rule-based
        # режиме (LLM здесь не вызывается, классифицировать больше некому).
    ...
    # В LLM-режиме тип документа определяет сам Claude/Gemini как часть
    # extraction tool-call (см. EXTRACTION_TOOL["documents"]) — без отдельного
    # regex-прохода до вызова.
...
async def _apply_llm_doc_classification(
    ...
    """Записать классификацию типов документов из tool-call (EXTRACTION_TOOL["documents"])
    в ClaimDocument и вернуть обновлённый список OCRResult + doc_score-флаги.

    Аналог classifier.reclassify_documents(), но источник — LLM, не regex
    (doc_type_source="llm"). doc_index сопоставляется с позицией документа
    в ocr_results (1-based, тот же порядок, что и в _build_user_message).
    """
    from sqlalchemy import select

    by_index: dict[int, dict] = {
        entry["doc_index"]: entry
        for entry in raw_documents
        if isinstance(entry.get("doc_index"), int)
    }
    ...
    threshold = settings.extraction_doc_type_low_confidence_threshold
    updated: list[OCRResult] = []
    flags: list[str] = []

    for position, ocr in enumerate(ocr_results, start=1):
        entry = by_index.get(position)
        if entry is None:
            log.warning("doc_classification_missing", claim_id=str(claim_id), doc_index=position)
            flags.append("unclassified_document")
            updated.append(ocr)
            continue

        try:
            doc_type = DocType(entry.get("doc_type"))
        except ValueError:
            log.warning(
                "doc_classification_invalid_type",
                claim_id=str(claim_id),
                doc_index=position,
                raw_type=entry.get("doc_type"),
            )
            doc_type = DocType.OTHER

        confidence = entry.get("doc_type_confidence") or 0.0
        if doc_type == DocType.OTHER:
            flags.append("unclassified_document")
        elif confidence < threshold:
            flags.append("low_confidence_doc_type")

        doc = docs_by_id.get(ocr.doc_id)
        if doc is not None:
            doc.doc_type = doc_type
            doc.doc_type_source = "llm"
            log.info(
                "doc_type_classified_by_llm",
                claim_id=str(claim_id),
                doc_id=str(ocr.doc_id),
                doc_type=doc_type.value,
                confidence=confidence,
            )

        if doc_type != ocr.doc_type:
            updated.append(OCRResult(
                ...
                low_confidence_blocks=ocr.low_confidence_blocks,
                strategy_used=ocr.strategy_used,
            ))
        else:
            updated.append(ocr)
...
async def _extract_with_claude(
    ...
    with AuditTimer() as timer:
        # Тип документа определяет сам Claude/Gemini в этом же tool-call
        # (EXTRACTION_TOOL["documents"]) — до вызова классификация не нужна.
        ...
        # Записываем LLM-классификацию типов документов в ClaimDocument и
        # получаем ocr_results с актуальным doc_type (нужно _persist_extracted_data
        # ниже — она ветвится по ocr.doc_type).
        ocr_results, doc_type_flags = await _apply_llm_doc_classification(
            raw.get("documents", []), ocr_results, db, claim_id, tenant_id,
        )
        ...
        try:
            ...
            extraction = ExtractionResult(
                ...
                flags=[*raw.get("flags", []), *doc_type_flags],
